...
class Layer(object):
    ...
    def backward(self, x, delta_prev):
        ...
        # backpropagation
        # Áp dụng CT 5
        # CT 3: deltal(l) = deltal(l + 1) * sigma'(z(l)) = deltal(l + 1) * sigma'(self.output)
        # CT 5: 
        
        if(self.activation == 'sigmoid'):
            delta = delta_prev*sigmoid_grad(self.output) # CT 3
            
        
        elif(self.activation == 'tanh'):
            delta = delta_prev*tanh_grad(self.output) # CT 3
            

        elif(self.activation == 'relu'):
            delta=delta_prev*reLU_grad(self.output)
        w_grad = np.dot(x.T,delta) # CT 5

        # [TODO 1.4] Implement L2 regularization on weights here
        w_grad +=  self.reg*self.w #CT 10
...
class NeuralNet(object):
    ...
    def backward(self, y, all_x):
        ...
        y_hat_last=all_x[-1]
        delta = y_hat_last - y # CT 8: delta(L) = s - y
        delta /= y.shape[0]
